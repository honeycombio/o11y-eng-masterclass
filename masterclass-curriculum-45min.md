...
**CI/CD pipelines (10 min, demo).** Chapter 18's claim: build observability has the best ROI of all observability applications. Your pipeline is a distributed system; steps are spans, runs are traces, failed steps are errors. Instrument a span per job step with OTel's own CI/CD semantic conventions — `cicd.pipeline.name`, `cicd.pipeline.task.name`, `cicd.pipeline.task.run.result`, `vcs.change.id` for the PR number — rather than inventing attribute names; one span per test case to query flakiness and duration. Define SLIs for build reliability the way you do for services. Live: a build-time regression from 12 to 22 minutes caught on day one by a P95-duration trigger instead of going unnoticed for a week — plus, time permitting, a real trace of this masterclass repo's own CircleCI pipeline via Honeycomb's buildevents tool, the point being how little it takes to add this to a pipeline you already run (that tool predates OTel semconv and uses its own field names; the semconv story stays with the seeded dataset above).
