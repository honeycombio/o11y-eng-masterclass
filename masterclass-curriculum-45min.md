...
**Where the money goes (8 min).** On a legacy, three-pillars vendor, three drivers: ingest volume (events/sec × event size/field count — many legacy vendors price this way, so every attribute you add compounds), retention (90 days costs ~3× what 30 does; are you actually querying old data or just hoarding it?), and query compute (auto-refreshing dashboards nobody watches burn budget silently). Name the Honeycomb differentiator explicitly here: Honeycomb bills per event, not per attribute or byte, so a wide event costs the same as a narrow one — MC1's "arbitrarily wide events" philosophy doesn't cost you extra on this platform. Retention and query compute are still real levers regardless of vendor; ingest-by-width is the one that's vendor-specific, so check your own bill rather than assuming either story.
