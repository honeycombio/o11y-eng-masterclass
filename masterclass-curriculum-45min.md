...
**LLM observability (12 min, demo).** Chapter 21 pairs two ideas that must work together: evaluations for reliability, and telemetry designed for LLM apps. The new challenges: non-determinism (success/failure can't capture quality, so you need evals), latency variability (P50 fine, P99 catastrophic, and users feel P99), cost per request (`gen_ai.usage.input_tokens` / `output_tokens` are non-negotiable; where the money figure lives is a genuine fork — Chapter 21 notes the conventions define no cost key and recommends deriving cost at query time from model and token counts, so re-pricing is retroactive, while the stage demo takes the other branch and stores `cost_usd` at emit time, which is simpler to query but freezes the price. I'll show both and let you pick), and multi-step pipelines (retrieval, assembly, LLM call, parsing; each its own span). Live: Claude Code's own OTel export, running on stage, showing that multi-step span hierarchy with real cost and token counts in real time — not a synthetic stand-in, since the tool doing the demo already emits exactly the telemetry the chapter describes. The loop closes when production telemetry feeds your evals and your next iteration; the AI-era version of observability-driven development.
