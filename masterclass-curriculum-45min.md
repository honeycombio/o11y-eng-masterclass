...
**Where the money goes (8 min).** Three drivers: ingest volume (events/sec — Honeycomb bills per event, not per attribute or byte, so a wide event costs the same as a narrow one; some other vendors do charge by size or field count, which is a real reason to check your own bill before assuming this session's advice transfers unchanged), retention (90 days costs ~3× what 30 does; are you actually querying old data or just hoarding it?), and query compute (auto-refreshing dashboards nobody watches burn budget silently).
