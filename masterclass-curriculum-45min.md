...
**Targets and burn-rate alerts (12 min).** Targets are a negotiation, not a calculation; start at or just above your 30-day baseline. Chapter 12's alert taxonomy: threshold-crossing, relative burn, and predictive burn alerts, with time framed as a sliding window. Relative burn is the *SRE Workbook* ratio mechanism the chapter cites directly — alert when failures in a short trailing window exceed what the error budget allows for that window, 1x being normal and 4x or more paging. The familiar multi-window pairing (2% of budget in 1 hour, 5% in 6 hours) comes from that same *Workbook* table rather than from Chapter 12; the chapter's own recommendation is the predictive burn alert, extrapolating a baseline window across a lookahead window so you alert *before* the budget is empty, not after.

**Event-based SLOs and why they win (10 min, demo).** Chapter 11 draws the event-based/time-based distinction and defers the reasoning to Chapter 12's "Using Structured Event Data for SLOs Versus Time-Series Data", which is the argument here. Metric-based SLOs tell you *that* something broke. Event-based SLOs tell you *what*, *for whom*, and *why*. Live: an alert fires, I draw a BubbleUp box around the errors — `error.type=dependency_unavailable` is the standout, since every error in this dataset carries that value, the *what*. Then I click into the breakdown query, sorted ascending, and `service.version=1.4.2` + `user.type=enterprise` is the worst row, the *for whom*. "SLO burning" to "I know what broke and who it's hitting" without leaving the page. The SLO and the debugger are the same system over the same data.

### Async lab (self-serve)

Write an SLI for a service you own and validate it against historical data in the workshop environment; sanity-check it with BubbleUp before committing. Try it twice, once by hand and once by prompting a model with Chapter 11's prompt, and diff the two — the gap is the review you'd otherwise skip. Then draft an error-budget policy: what happens at 50% burned, at 100%, who gets paged and what action follows. Tooling can't enforce what the org hasn't decided.
...
**Where the money goes (8 min).** On a legacy, three-pillars vendor, three fronts to fight: ingest volume (events/sec × event size/field count, so every attribute you add compounds), retention (priced roughly linear with duration), and query compute (billed per query or per dashboard refresh). On Honeycomb specifically, it's really one front. Ingest volume is still the real lever — but billed per event, not per attribute or byte, so MC1's "arbitrarily wide events" philosophy doesn't cost extra here; fewer events is where the money actually is, which is the whole reason this session spends most of its time on sampling. Storage is S3-backed and cheap (Honeycomb's own cost is roughly 2.5¢/GB plus a markup), included free by default well past 30 days, so it isn't a lever worth fighting the way a legacy vendor's duration-multiplied retention pricing is. Query compute is free for humans, full stop, on Honeycomb — unlike vendors who bill per query or per refresh; there's a live nuance worth a verbal aside about automated/agentic querying that doesn't belong in writing here.

**Sampling, the biggest lever (18 min, live demo).** Chapter 15 builds it up, rung by rung: the base case, fixed-rate, recording the sample rate, consistent sampling, target-rate, more than one static rate, sampling by key and target rate, dynamic rates over arbitrarily many keys, and finally combined head-and-tail per-key target-rate sampling. The two static rates matter more than they look — that's the rung where you stop treating all traffic alike, and every per-key strategy after it is a generalisation of the same move. The principles: record the sample rate so the query engine corrects for it, and reduce what you never query. Chapter 4 supplies the one worth repeating to a metrics-heavy org — a metric *is* a form of sampling, pre-aggregated at emit time and discarding the events underneath, so deriving metrics from events beats running a parallel metrics pipeline. Sampling is cost control with preserved statistical accuracy, not data loss. Head-based at a fixed rate is the simple default; tail-based per-key now runs as a single Collector processor (Honeycomb's `adaptive_tail_sampling`, built on the same adaptive algorithms as Refinery) rather than a separate stateful service — that last part is our product history, not the chapter's: Chapter 15 is deliberately vendor-neutral and never mentions Refinery, so flag it as a product aside rather than something attendees will find in the book — keep 100% of errors, sample the happy path hard. Live, on the seeded dataset: keep-all-errors (15% of traffic) plus a 5%-of-the-rest catch-all retains ~19% of events — an ~81% reduction, US$50k/month down to about US$9.5k/month, with no meaningful loss of debugging power (every error, none of the redundant healthy volume). The trap: even Collector-based tail sampling needs per-trace buffering and adds latency to the pipeline; below ~10k traces/sec, head-based is probably right, and this processor is still upstream `development` stability — verify behavior before depending on it in production. Don't over-engineer before you need to. Worth saying out loud that this is me disagreeing with our own Chapter 4, which argues sampling is best practice at any scale and that simple probability-based head sampling has real drawbacks below enormous scale. The statistical point is correct: a flat rate at low volume can drop the only error you needed. My claim is narrower and operational — what you shouldn't run below ~10k traces/sec is a *stateful tail-sampling tier*; sample by key at the head instead of at a flat rate, and you get content-awareness without the buffering.

**Pipelines and storage (12 min).** Chapter 16's telemetry pipeline functions; collect, normalise/secure, enrich, reduce, route. Reduce and route are where cost control lives: drop and transform before the backend, route signals to different destinations with different retention. Dataset design tradeoffs (monolithic for easy cross-service queries vs. per-service for isolation and independent sampling policy — not retention, which Honeycomb doesn't vary per dataset). And the self-hosted question (Chapters 13–14): ClickHouse gives you the concepts to evaluate it honestly, but the answer is usually "not until operating it costs less than the licensing delta"; do the full TCO including on-call and the product features you won't build.

**Organisational discipline (7 min).** On a legacy vendor, retention is a lever you actively manage for cost: short windows for non-prod, delete old data before storage charges climb. On Honeycomb, that's not how it works. Full unsampled data archives to cold S3 storage automatically, and Enhance/Rehydrate pulls it back on demand — either older data past standard retention, or full fidelity on traffic you sampled out. Sampling hard isn't a one-way door: you're choosing what stays hot and indexed by default, not permanently discarding anything. What's left as real discipline here: review sampling rates per service as traffic shifts (the actual lever, per the section above), periodic board/query cleanup for focus rather than cost, and cost as a forcing function for adoption (Chapter 27) — the monthly bill review question is "is ingest volume trending the way we expect," not "are we hoarding data."
...
**CI/CD pipelines (10 min, demo).** Chapter 18's claim: build observability has the best ROI of all observability applications. Your pipeline is a distributed system; steps are spans, runs are traces, failed steps are errors. The chapter gets there by way of an ontology (Chapter 17's verbs and nouns): a workflow is one trace, each job a span beneath it, each step a span within the job. This session pins that ontology to OTel's CI/CD semantic conventions — `cicd.pipeline.name`, `cicd.pipeline.task.name`, `cicd.pipeline.task.run.result`, `vcs.change.id` for the PR and `vcs.ref.head.name` for the branch — rather than inventing attribute names; one span per test case to query flakiness and duration. The chapter singles out repository and branch as the most important telemetry a pipeline emits, and the demo shows why: the same flaky test at the same rate is one author's problem on a PR branch and everyone's on the trunk. Define SLIs for build reliability the way you do for services. Live: a build-time regression from 12 to 22 minutes caught on day one by a P95-duration trigger instead of going unnoticed for a week.

**Frontend and mobile (8 min).** Chapter 19; you don't control the environment, the network is unreliable, and the user is a real human whose session you reconstruct. The pattern is two halves, and the second is the one people miss: a persisted, high-performance *circular buffer* on the device, so developers can log freely because logs don't leave by default — plus a real-time bidirectional *control plane* that tells each device what to send and under what conditions. That's the fidelity dial: a state machine pushed from server to client ("if the user leaves the checkout screen, or 15 seconds pass without a checkout, dump the session") means you send a lot of data when you need it and none when you don't, changeable without an app release. Session correlation: propagate a session identifier from frontend through every backend service. But the chapter's sharper point is that backend-side stitching is *insufficient* — knowing every request from a session still can't tell you why there's a two-minute gap between two of them. Real example: the chapter's FakeBurger delivery app, where checkout rate is the product metric, and the findings were things no error log ever recorded — a "Next" button rendering below the fold, and a new discount-code format that disabled Submit until the user backed out and re-entered.

**Performance engineering (10 min, demo).** Chapter 20; production traffic is your most accurate benchmark, not synthetic load. The workflow I'll run: find expensive operations (`P99(duration_ms) GROUP BY db.query.text`), characterise the distribution (a bimodal heatmap is a different problem than a long tail), correlate with attributes (`AVG(duration_ms) GROUP BY user.type`), and track optimisation impact across deploy markers. But the chapter opens somewhere else, and it's worth saying why: a mystery millisecond on every ingest request, invisible to our own tracing, which we found with a Go pprof flame graph — `regexp.Compile` inside the router, recompiled per request. "We're big fans of distributed tracing, but it's not the best tool for every job"; below single-digit milliseconds you need profiling, and knowing which signal answers which question is the actual skill. Worked example: our own Graviton/ARM64 migration, validated on production data — roughly 30% fewer instances, 40% lower EC2 cost, no latency regression confirmed by P50/P99 comparison. Those are Honeycomb's numbers from our migration, not the chapter's; Chapter 20's own figure is that each Arm generation shows about 30% price-performance gains. We trusted the production data, not the benchmark.

**LLM observability (12 min, demo).** Chapter 21 pairs two ideas that must work together: evaluations for reliability, and telemetry designed for LLM apps. The new challenges: non-determinism (success/failure can't capture quality, so you need evals), latency variability (P50 fine, P99 catastrophic, and users feel P99), cost per request (`gen_ai.usage.input_tokens` / `output_tokens` are non-negotiable; where the money figure lives is a genuine fork — Chapter 21 notes the conventions define no cost key and recommends deriving cost at query time from model and token counts, so re-pricing is retroactive, while the stage demo takes the other branch and stores `cost_usd` at emit time, which is simpler to query but freezes the price. I'll show both and let you pick), multi-step pipelines (retrieval, assembly, LLM call, parsing; each its own span), and user feedback as telemetry — Chapter 21's point that errors and response time are poor proxies for UX here, because a fast wrong answer still fails the user, so thumbs up/down and user-retry belong on the span as first-class attributes and double as a source of new evals. Live: Claude Code's own OTel export, running on stage, showing that multi-step span hierarchy with real cost and token counts in real time — not a synthetic stand-in, since the tool doing the demo already emits exactly the telemetry the chapter describes. The loop closes when production telemetry feeds your evals and your next iteration; the AI-era version of observability-driven development.
